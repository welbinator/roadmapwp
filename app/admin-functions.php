...
function enqueue_admin_styles($hook): void
{
    ...
    if ('post.php' == $hook && isset($post) && 'idea' == $post->post_type) {
        ...
        wp_enqueue_style('wp-roadmap-idea-admin-styles', $css_url, array(), RMWP_FREE_PLUGIN_VERSION);
    }
    ...
    if ($hook == 'roadmap_page_wp-roadmap-taxonomies') {
        ...
        wp_enqueue_style('wp-roadmap-general-admin-styles', $css_url, array(), RMWP_FREE_PLUGIN_VERSION);
    }

    // Enqueue CSS for help page
    if ($hook == 'roadmap_page_wp-roadmap-help') {
        $tailwind_css_url = plugin_dir_url(__FILE__) . '../dist/styles.css';
        wp_enqueue_style('wp-roadmap-tailwind-styles', $tailwind_css_url, array(), RMWP_FREE_PLUGIN_VERSION);
        wp_enqueue_script('my_custom_script', plugin_dir_url(__FILE__) . 'assets/js/help.js', array('jquery'), RMWP_FREE_PLUGIN_VERSION, true);
    }

    // Enqueue JS for the 'Taxonomies' admin page
    if ('roadmap_page_wp-roadmap-taxonomies' == $hook) {
        wp_enqueue_script('wp-roadmap-taxonomies-js', plugin_dir_url(__FILE__) . 'assets/js/taxonomies.js', array('jquery'), RMWP_FREE_PLUGIN_VERSION, true);
        wp_localize_script(
...
add_action('admin_enqueue_scripts', __NAMESPACE__ . '\\enqueue_admin_styles', 10);
...
function enqueue_frontend_styles(): void
{
    ...
    if ($force_enqueue || check_for_shortcode_or_block_presence()) {
        ...
        wp_enqueue_style('wp-roadmap-frontend-styles', $custom_css_url, array(), RMWP_FREE_PLUGIN_VERSION);

        // Enqueue and localize scripts as necessary
        wp_enqueue_script('wp-roadmap-voting', plugin_dir_url(__FILE__) . 'assets/js/voting.js', array('jquery'), RMWP_FREE_PLUGIN_VERSION, true);
        wp_localize_script('wp-roadmap-voting', 'RoadMapWPVotingAjax', array('ajax_url' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('wp-roadmap-vote-nonce')));

        wp_enqueue_script('wp-roadmap-idea-filter', plugin_dir_url(__FILE__) . 'assets/js/idea-filter.js', array('jquery'), RMWP_FREE_PLUGIN_VERSION, true);
        ...
        wp_enqueue_script('wp-roadmap-roadmap-tabs', plugin_dir_url(__FILE__) . 'assets/js/roadmap-tabs.js', array('jquery'), RMWP_FREE_PLUGIN_VERSION, true);
        wp_localize_script('wp-roadmap-roadmap-tabs', 'roadmapwpAjax', array('ajax_url' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('roadmap_nonce')));
        
        do_action('enqueue_pro_frontend_styles'); // Allow the pro version to enqueue frontend scripts/styles
    }
}
...
/**
 * Redirect to the correct page for 'idea' post types.
 */
